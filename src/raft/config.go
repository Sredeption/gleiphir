...
func makeConfig(t *testing.T, n int, unreliable bool) *config {
	...
	cfg.saved = make([]*data.MemoryPersister, cfg.n)
...
func (cfg *config) crash1(i int) {
	...
	if cfg.saved[i] != nil {
		raftLog := cfg.saved[i].ReadState()
		cfg.saved[i] = data.MakeMemoryPersister()
		cfg.saved[i].SaveState(raftLog)
	}
}
...
func (cfg *config) start1(i int) {
	...
	ends := make([]*mock.ClientEnd, cfg.n)
	for j := 0; j < cfg.n; j++ {
	...
	peers := new(mock.Peers)
	peers.Ends = ends
	rf := Make(peers, i, cfg.saved[i], applyCh)

	cfg.mu.Lock()
	cfg.rafts[i] = rf
	cfg.mu.Unlock()

	svc := mock.MakeService(rf)
	srv := mock.MakeServer()
	srv.AddService(svc)
